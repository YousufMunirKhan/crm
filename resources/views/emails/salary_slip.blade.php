...
<head>
    ...
    <title>Your Salary Slip - {{ $month }}</title>
    <style>
        body {
...
<body>
    <div class="email-container">
        ...
        <div class="content">
            ...
            <p class="message">Please find your salary slip for <strong>{{ $month }}</strong> attached to this email.</p>

            <div class="slip-box">
                @php
                    $currencySymbol = $salary->currency === 'PKR' ? 'Rs' : '£';
                @endphp
                <p><strong>Net Salary:</strong> {{ $currencySymbol }}{{ number_format($salary->net_salary, 2) }} ({{ $salary->currency }})</p>
                @if($salary->attendance_days)
                <p><strong>Attendance Days:</strong> {{ $salary->attendance_days }} days</p>
                @endif
                <p><strong>Month:</strong> {{ $month }}</p>
            </div>

            <p class="message">Please download your salary slip from the attachment below. The PDF is attached to this email.</p>

            <p class="attachment-note">
                📎 The salary slip document is attached to this email. You can open it directly or save it for your records.
            </p>

            <p class="message" style="margin-top: 24px;">If you have any questions regarding your salary, please contact the HR department.</p>
        </div>

        <div class="footer">
            <p class="company-name">{{ $companyName }}</p>
            @if($companyAddress ?? null)
                <p class="contact-info">📍 {{ strip_tags($companyAddress) }}</p>
            @endif
            ...
            <p class="footer-note">This is an automated message. HR Department · {{ $companyName }}</p>
        </div>
    </div>
</body>
</html>
